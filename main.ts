function askForString (message: string, answerLength: number, useOnScreenKeyboard: boolean) {
...
// % whenUsed=true
let CELL_WIDTH = Math.floor(CONTENT_WIDTH / ALPHABET_ROW_LENGTH)
// % whenUsed=true
...
let BLANK_PADDING = 1
// % whenUsed=true
let ROW_LEFT = PADDING + Math.floor((CONTENT_WIDTH - CELL_WIDTH * ALPHABET_ROW_LENGTH) / 2)
// Dimensions of the bottom bar
// % whenUsed=true
let BOTTOM_BAR_ALPHABET_MARGIN = 4
// % whenUsed=true
let BOTTOM_BAR_HEIGHT = PADDING + BOTTOM_BAR_ALPHABET_MARGIN + CELL_HEIGHT
// % whenUsed=true
let BOTTOM_BAR_BUTTON_WIDTH = PADDING * 2 + font.charWidth * 3
// % whenUsed=true
...
// % whenUsed=true
let CONFIRM_BUTTON_LEFT = screen.width - BOTTOM_BAR_BUTTON_WIDTH
...
// % whenUsed=true
let INPUT_TOP = ALPHABET_TOP - INPUT_HEIGHT - ALPHABET_INPUT_MARGIN
// % whenUsed=true
let lowerShiftText = "ABC"
// % whenUsed=true
let upperShiftText = "abc"
// % whenUsed=true
digitsUpper = [
" ",
",",
...
class Prompt {
        ...
        protected drawInputArea() {
            ...
            if (this.useSystemKeyboard) {
                if (this.selectionStart === this.selectionEnd && this.selectionStart < this.answerLength) {
                    const col2 = this.selectionStart % ALPHABET_ROW_LENGTH;
                    const row2 = Math.floor(this.selectionStart / ALPHABET_ROW_LENGTH);
                    if (!this.blink()) {
                        screen.fillRect(
                            answerLeft + col2 * CELL_WIDTH,
                            INPUT_TOP + row2 * CELL_HEIGHT,
                            1,
                            CELL_HEIGHT,
                            this.theme.colorCursor
                        );
        ...
        protected drawKeyboard() {
            ...
            for (let j = 0; j < this.keyboardRows * this.keyboardColumns; j++) {
                const col3 = j % this.keyboardColumns;
                const row3 = Math.idiv(j, this.keyboardColumns);

                if (col3 === this.cursorColumn && row3 === this.cursorRow) {
                    screen.fillRect(
                        left + col3 * CELL_WIDTH,
                        top + row3 * CELL_HEIGHT,
                        CELL_WIDTH,
                        CELL_HEIGHT,
                        this.theme.colorCursor
                    )
